/*******************************************************/
/*           Лабораторная  работа ╧2                   */
/*          Типы данных и ввод-вывод                   */
/*        Пример выполнения. Вариант ╧30.              */
/*******************************************************/
#include<iostream>
#include <stdio.h>
int main(void) 
{
	setlocale(LC_ALL, "Rus");
	int name1[9], name2[9], name3[9];
	char sc1, sc2, sc3;
	unsigned short cnt1, cnt2, cnt3;
	float sq1, sq2, sq3;
	/* Введение фактических данных*/
	printf("1. Введите: год, научного руководителя, Диаметр антенны(м), Рабочая частота(МГц) >");
	scanf_s("%d %c %d %f", name1, &sc1, &cnt1, &sq1);
	printf("2. Введите: название, школу, количество, площадь >");
	scanf_s("%d %c %d %f", name2, &sc2, &cnt2, &sq2);
	printf("3. Введите: название, школу, количество, площадь >");
	scanf_s("%d %c %d %f", name3, &sc3, &cnt3, &sq3);
	/* Вывод таблицы */
	/* вывод заголовков */
	printf("-----------------------------------------------------------------------\n");
	printf("|                 Проекты поиска внеземных сигналов                   |\n");
	printf("|---------------------------------------------------------------------|\n");
	printf("| Год | Научный руководитель | Диаметр антенны(м)|Рабочая частота(МГц)|\n");
	printf("|     |                      |                   |                    |\n");
	printf("|-----|----------------------|-------------------|--------------------|\n");
	/* вывод строк фактических данных */
	printf("| %9s | %c | %3d | %-5.1f |\n", name1, sc1, cnt1, sq1);
	printf("| %9s | %c | %3d | %-5.1f |\n", name2, sc2, cnt2, sq2);
	printf("| %9s | %c | %3d | %-5.1f |\n", name3, sc3, cnt3, sq3);
	/* вывод примечаний */
	printf("|-------------------------------------------------------------------|\n");
	printf("| Примечание: наблюдались объекты от 2 звезд до нескольких галактик |\n");
	printf("| до нескольких галактик от 2 звезд до нескольких галактик          |\n");
	printf("---------------------------------------------------------------------\n");
	return 0;
}


